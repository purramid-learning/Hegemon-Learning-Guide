...
This is the solution you see in most ed-tech chatbot tutors. The LLM agent begins a chat with the student, asking successive questions to narrow in on the problem. The agent has no understanding of where the student is in their lesson or quiz. Every chat begins with interrogation, meaning any attempt to help requires multiple responses to reach the current friction point. Each required response that doesn't directly address the issue is an opportunity for the student to quit. The more opportunities there are, the more likely they are to quit.
